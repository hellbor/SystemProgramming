...
public:
	...
	void accelerate()
	{
		...
		{
			speed += acceleration;
			if (!threads_conteiner.free_wheeling_thread.joinable())
				threads_conteiner.free_wheeling_thread = std::thread(&Car::free_weeling, this);
	...
	void control()
	{
		...
		do
		{
			...
			if (speed <= 0 && threads_conteiner.free_wheeling_thread.joinable())
				threads_conteiner.free_wheeling_thread.join();
		} while (key != Escape);
